...
myApp.ig.getMediaByLocationID = function(id) {
...
myApp.ig.init = function() 
{
	...
	alert(`Since Instagram's API is very limited, please only search Baldwin Village Toronto. (If your app is not live, Instagram only allows you to retrieve information on a few users who you invite to test your app in "sandbox mode". I have created a demo account with demo posts.`);
}
...
myApp.ig.pushPost = function(item) {
	...
	if (!(myApp.ig.currPostsLocations.includes(item.location)))
	{
		myApp.ig.currPostsLocations.push(item.location);
...
myApp.ig.updateGallery = function() {
	...
	if (myApp.ig.currHashtag != "") 
	{
		...
		{
			myApp.ig.pushPost(myApp.ig.currPosts[i]);
		}
	}
	else 
	{
